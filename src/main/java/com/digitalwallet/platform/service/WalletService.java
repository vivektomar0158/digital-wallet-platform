...
public class WalletService {
    ...
    public WalletStatementResponse getWalletStatement(
        ...
        BigDecimal totalWithdrawals =
            filteredTransactions.stream()
                .filter(
                    tx -> tx.getFromWallet() != null && tx.getFromWallet().getId().equals(wallet.getId()))
                .map(Transaction::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    public TransferResponse initiateTransfer(
        ...
        if (transactionProducer != null) {
            ...
            log.debug(
                "TransactionProducer bean not available — skipping async send for transaction {}", referenceId);
        }
