...
public class WalletService {
  ...
  // Injected Producer
  private final com.digitalwallet.platform.service.messaging.TransactionProducer
      transactionProducer;
  ...
  public TransferResponse initiateTransfer(
    ...
    Transaction transaction =
        Transaction.builder()
            ...
            // We don't fetch receiver wallet yet to keep this lightweight
            // But we just need to ensure the number exists later.
            // Actually, for FK integrity we probably want to fetch it or store the number
            // loosely?
            // Let's fetch it to ensure it exists before we queue it.
            .toWallet(
                walletRepository
  ...
  public Transaction executeAsyncTransfer(TransactionEvent event) {
    ...
    try {
       ...
       * throw e;
       */
      // Since transaction is fetched with orElseThrow, it's never null here.
      transaction.setStatus(TransactionStatus.FAILED);
      transaction.setFailureReason(e.getMessage());
      return transactionRepository.save(transaction);
    }
